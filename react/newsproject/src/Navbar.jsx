...
const LoginPopup = ({ onClose }) => {
  return (
    <div
      className="absolute right-0 top-10 w-72 bg-gray-800 text-white p-4 rounded-xl shadow-lg z-50 transition-all ease-in-out duration-300"
      style={{ zIndex: 9999 }} // Ensure it's above other elements
    >
      {/* Staff login as external link */}
      <button
        onClick={() => {
          console.log("Staff login clicked");
          window.location.href = "http://localhost/nepnews/role-login.php";
        }}
        className="block font-semibold text-sm mb-1 hover:text-emerald-300"
      ...
      <button
        onClick={() => {
          ...
          window.location.href = "http://localhost/nepnews/admin_login.php";
        }}
        className="block font-semibold text-sm mb-1 hover:text-emerald-300"
        style={{ cursor: 'pointer', zIndex: 9999 }} // Added z-index to ensure it's above
      >
        Admin Login
      </button>
    ...
    <header className="fixed top-0 w-full z-[1000] bg-white shadow p-4 transition-all flex items-center justify-between">
      ...
      <div className="relative ml-auto">
        <form onSubmit={handleSearch}>
          <input
            className="w-full max-w-xs bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md pl-3 pr-28 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
